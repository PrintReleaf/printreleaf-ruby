...
describe PrintReleaf::QuoteItem, "properties" do
  let(:json) do
    JSON.parse <<-JSON
      {
        "quantity": 20000,
  ...
  it "gives access to JSON properties" do
    item = PrintReleaf::QuoteItem.new(json)
    expect(item.quantity).to eq 20000
...
describe PrintReleaf::QuoteItem, "#paper_type" do
